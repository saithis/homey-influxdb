...
module.exports = class InfluxDb extends EventEmitter {
    ...
    async updateSettings(options) {
        ...
        this._isV3 = this._organization && this._organization.length == 0 && this._token && this._token.length > 0;
    }
    ...
    async _pingDatabase(host, protocol, port, token) {
        return this._isV2 || this._isV3 ? await this._pingDatabaseV2V3(host, protocol, port, token) :
    ...
    async _pingDatabaseV2V3(host, protocol, port, token) {
        try {
            await this._pingDatabaseV1(host, protocol, port);
            return;
        } catch (err) {
            // V1 ping failed, try V2/V3 health endpoint
        }
        let result;
        try {
            result = await http.get({
                ...
                timeout: 5000,
                json: false, // Don't auto-parse as JSON (v3 returns plain text)
                headers: {
                    'Authorization': `Token ${token}`
                }
            });
        } catch (err) {
            throw new Error(`Failed to check health of InfluxDB v2/v3 at ${protocol}://${host}:${port}: ${err.message}`);
        }
        if (!result || !result.response) {
        ...
        if (result.response.statusCode === 200) {
            ...
            if (typeof result.data === 'string' && result.data.trim() === 'OK') {
                return;
            }
            // InfluxDB v2 returns JSON with status field
            try {
                const data = typeof result.data === 'string' ? JSON.parse(result.data) : result.data;
                if (data.status === 'pass') {
                    return;
                }
                throw new Error(`InfluxDb at ${protocol}://${host}:${port} health check failed: ${data.status}`);
            } catch (parseErr) {
                // If JSON parsing fails but we got 200, accept it
                if (result.data) {
                    return;
                }
                throw new Error(`InfluxDb at ${protocol}://${host}:${port} returned unexpected response: ${result.data}`);
            }
        } else {
            throw new Error(`InfluxDb at ${protocol}://${host}:${port} is not online! (${result.response.statusCode})`);
        }
    ...
    _writePoints(measurements) {
        // v3 has backwards compatibility with v2, so we use the same function for both
        return this._isV2 || this._isV3 ? this._writePointsV2(measurements) : this._writePointsV1(measurements);
    }
