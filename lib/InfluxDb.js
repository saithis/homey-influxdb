...
module.exports = class InfluxDb extends EventEmitter {
    ...
    async _pingDatabase(host, protocol, port, token) {
        if(this._version === 'v1') {
            return await this._pingDatabaseV1(host, protocol, port);
        }

        return await this._pingDatabaseV2V3(host, protocol, port, token);
    }
    ...
    _writePoints(measurements) {
        if(this._version === 'v1') {
            return this._writePointsV1(measurements);
        }
        // v3 has backwards compatibility with v2, so we use the same function for both (v2 and v3)
        return this._writePointsV2(measurements);
    }
